<template>
  <div>
    <q-dialog v-model="$state.updateDialog" maximized transition-show="slide-up" transition-hide="slide-down">
      <q-card>
  ...
  verdana: 'Verdana'
            ...
            <div class="q-ml-md text-body2 text-red-8" v-show="v$.description.$error">{{ v$.description.$errors.map((e) =>
              e.$message).join()
            }}</div>
          </q-card-section>
...
const { $state, updateTrainer } = useTrainerStore();
const loading = ref(false)
const id = ref()
const imageUrl = ref();
const formDialog = ref<Trainer>({
  name: '',
  qualification: '',
  description: '',
  image: null,
...
const onSubmit = async () => {
  if (!v$.value.$invalid) {
    try {
      loading.value = true;
      await updateTrainer(id.value, formDialog.value)
...
watchEffect(() => {
  formDialog.value.name = $state.trainer.name
  formDialog.value.email = $state.trainer.email
  formDialog.value.qualification = $state.trainer.qualification
  imageUrl.value = storageBaseUrl + $state.trainer.image
  formDialog.value.description = $state.trainer.description
