<template>
      ...
      <q-card>
        <q-card-section class="bg-primary text-white">
          <div class="row justify-between">
            <div class="col-6">
              <div class="text-h6">Tambah Materi untuk kelas {{ props.courseName }}</div>
            </div>
            <div class="col-1">
  ...
  verdana: 'Verdana'
            ...
            <div class="q-ml-md text-body2 text-red-8" v-show="v$.description.$error">{{ v$.title.$errors.map((e) =>
              e.$message).join()
            }}</div>
          </q-card-section>
...
const { $state, addMaterial } = useMaterialStore();
const loading = ref(false)
const props = defineProps(['courseName', 'courseId'])
...
const onSubmit = async () => {
  if (!v$.value.$invalid) {
    try {
      loading.value = true;
      await addMaterial({ ...formDialog.value, course_id: props.courseId });
